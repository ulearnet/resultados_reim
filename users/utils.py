...
def get_from_db():
    db = pymysql.connect("ulearnet-db.cmfamk37bb89.sa-east-1.rds.amazonaws.com","masterulearnet","KsclS$AcSx.20Cv83xT","ulearnet_reim_pilotaje")
    cursor = db.cursor()
    return cursor
...
def get_timerpromsess_PYL(request):
...
def get_activities_colorPYL(request):
    ...
    start_base = 'SELECT u.id, concat(u.nombres ," ", u.apellido_paterno ," ", u.apellido_materno) as nombre, count(if(a.id_elemento=280100,1,NULL)) Rojo, count(if(a.id_elemento=280101,1,NULL)) Azul, count(if(a.id_elemento=280102,1,NULL)) Verde, count(if(a.id_elemento=280103,1,NULL)) Amarillo,count(if(a.id_elemento=280104,1,NULL)) Negro FROM alumno_respuesta_actividad a, usuario u, pertenece b  WHERE' + date
    final_base = ' a.id_user= u.id && b.usuario_id = a.id_user' + query_params + ' GROUP BY u.id;'

    return start_base + final_base

def get_activities_colortimePYL(request):
    query_params = ''
    ...
    start_base = 'SELECT concat(u.nombres ," ", u.apellido_paterno ," ", u.apellido_materno) as nombre, concat(day(datetime_touch),"/",month(datetime_touch),"/", year(datetime_touch)) AS fecha, count(if(a.id_elemento=280100,1,NULL)) Rojo, count(if(a.id_elemento=280101,1,NULL)) Azul, count(if(a.id_elemento=280102,1,NULL)) Verde, count(if(a.id_elemento=280103,1,NULL)) Amarillo,count(if(a.id_elemento=280104,1,NULL)) Negro FROM alumno_respuesta_actividad a, usuario u, pertenece b  WHERE' + date
    final_base = ' a.id_user= u.id && b.usuario_id = a.id_user' + query_params + ' GROUP BY day(a.datetime_touch) ORDER BY a.datetime_touch ASC;'
...
def get_ElemVisualTimer_PYL(request):
    cursor = get_from_db()
    query_params = ''
    ...
    start_base = ' SELECT concat(u.nombres ," ", u.apellido_paterno ," ", u.apellido_materno) as nombre, concat(day(datetime_touch),"/",month(datetime_touch),"/", year(datetime_touch)) AS fecha, count(if(a.id_elemento=280106,1,NULL)) Intensidad, count(if(a.id_elemento=280107,1,NULL)) Grosor, count(if(a.id_elemento=280105,1,NULL)) Borrar FROM alumno_respuesta_actividad a, usuario u, pertenece b WHERE' + date
    final_base = ' a.id_user= u.id && b.usuario_id = a.id_user ' + query_params +  ' GROUP BY day(a.datetime_touch) ORDER BY a.datetime_touch ASC;'
